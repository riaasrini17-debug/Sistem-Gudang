...
<head>
    ...
    <title>Kategori Barang - StockWise</title>
...
<body class="bg-gray-50 font-sans text-gray-900 h-screen overflow-hidden">
    <div class="flex h-full w-full">
        <aside class="w-64 bg-white border-r border-gray-100 flex flex-col shrink-0 h-full">
            ...
            <nav class="flex-1 p-4 space-y-1">
                ...
                <a href="{{ route('kategori.index') }}" class="flex items-center gap-3 px-4 py-3 bg-gray-900 text-white rounded-2xl text-sm font-medium shadow-xl">
        ...
        <main class="flex-1 flex flex-col h-full overflow-hidden">
            <header class="h-16 flex items-center justify-between px-8 border-b border-gray-50 bg-white shrink-0">
                <div class="font-black text-gray-900 tracking-tighter uppercase">Manajemen Kategori</div>
            ...
            <div class="flex-1 p-8 flex flex-col overflow-hidden bg-gray-50/30">
                <div class="flex-1 bg-white rounded-[2.5rem] border border-gray-100 shadow-sm flex flex-col overflow-hidden h-full">
                    ...
                    <div class="p-6 border-b border-gray-50 flex justify-between items-center shrink-0">
                        <div>
                            <h3 class="text-base font-black text-gray-900">Daftar Kategori Barang</h3>
                            <p class="text-xs text-gray-400">Kelompokkan barang Anda untuk mempermudah pencarian stok.</p>
                        </div>
                    ...
                    <div class="flex-1 overflow-y-auto custom-scrollbar">
                        <table class="w-full text-left table-fixed">
                            ...
                            <tbody class="divide-y divide-gray-50">
                                ...
                                @empty
                                <tr><td colspan="3" class="px-8 py-20 text-center text-gray-300 font-bold italic uppercase tracking-widest">Belum ada data kategori</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
    ...
    <div id="kategoriModal" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-50 p-4">
        <div class="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl">
            <h3 id="modalTitle" class="text-lg font-black mb-6 text-gray-900">Tambah Kategori</h3>
            <form id="kategoriForm" action="/admin/kategori/simpan" method="POST" class="space-y-4">
                @csrf
                <div id="methodContainer"></div>
                
                <div>
                    <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1 ml-1">Nama Kategori</label>
                    <input type="text" name="nama_kategori" id="input_nama" required class="w-full bg-gray-50 border-none rounded-2xl px-4 py-3 text-sm focus:ring-2 focus:ring-blue-600 outline-none" placeholder="Contoh: Elektronik">
                </div>
                <div>
                    <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1 ml-1">Deskripsi (Opsional)</label>
                    <textarea name="deskripsi" id="input_deskripsi" class="w-full bg-gray-50 border-none rounded-2xl px-4 py-3 text-sm focus:ring-2 focus:ring-blue-600 outline-none h-24" placeholder="Keterangan kategori..."></textarea>
                </div>
